          ...
          <div class="info_table onetime_pay_table">
            <table>
              ...
              <tr>
                ...
                <td class="table_price">{{return_one(count)}}매</td>
              </tr>
              <tr class="total_price">
                <td>결제금액</td>
                <td class="fontBold fRed">{{return_one(menu_fee * count)}}원</td>
              </tr>
            </table>
          </div>
        </section>
...
export default {
  ...
  mounted(){
    if(!JSON.parse(localStorage.getItem("third_menu")))
      ...
      this.third_menu = "N"
    this.tot_fee = this.menu_fee * this.count;
  ...
  methods : {
    before_reset(){
      ...
      this.$router.push({name : 'PayFleetPrepay'});
    },
    pay(){
      if(this.menu_fee == 0){
            ...
            (res) => {  
              ...
              (res) => {
                  if(res.data.resCd == "0000"){
                    this.waiting = false;
                    localStorage.setItem("is_type","fleetPrepay");
